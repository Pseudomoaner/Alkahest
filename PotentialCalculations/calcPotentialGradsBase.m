function [gradXYZ,gradTheta,gradPhi] = calcPotentialGradsBase(inField)
%CALCPORENTIALGRADSBASE uses only inbuilt Matlab commands to calculate the 
%potential gradients.
%
%   INPUTS:
...
for i = 1:length(inField.nCells) %The index of the cell alpha.
    ...
    if strcmp(inField.boundConds,'none')
        [dUdx,dUdy,dUdthet] = baseCalcEnergyGradients(xBets,yBets,lBets,nBets,thetBets,xAlph,yAlph,lAlph,nAlph,thetAlph,U0,lam);
    elseif strcmp(inField.boundConds,'periodic')
        [dUdx,dUdy,dUdthet] = baseCalcEnergyGradientsPeriodic(xBets,yBets,lBets,nBets,thetBets,xAlph,yAlph,lAlph,nAlph,thetAlph,U0,lam,boundX,boundY,Width,Height);
    end
